...
while easteregg !="jupi":
    ...
    if namen == "micycle" or namen =="Micycle" or namen =="minycle" or namen =="Minycle" or namen =="minicycle" or namen =="Minicycle" or namen =="mikeybikey" or namen =="Mikeybikey" or namen =="dinimueter":
        time.sleep(1)
        print("\nGratuliere!\nDu hast das unlösbare Game geknackt!")
        time.sleep(1)
        ...
        class Früchtekorb:
            ...
            herkunft=["der Schweiz","dem Ausland","dem Atlantis","dem Mars","der Erdatmosphäre","Spanien"]
            dedoraliv=False

            def __init__(self):
                while True:
                    try:
                        anzahl=int(input("\nWie viele Früchte sollen in den Früchtekorb? "))
                    except ValueError:
        ...
        if z.dedoraliv:
            ...
            if (damokles%2)==0:
                ...
                if x.dedoraliv2:
                    time.sleep(3)
                    print("\nNach diesem tollen Zirkusauftritt gibst du den Artisten Früchte von deinem Früchtekorb.")
                    time.sleep(2)
                    print("\nDie Früchte waren aber alle schon schimmelig.")
                    time.sleep(1)
                    print("\nDer Zirkus hat nicht überlebt.")
                    a.asciiskullded()
                if not x.dedoraliv2:
                        ...
                        a.asciikommunist()
                        time.sleep(1)
                        print("Der Zirkus hat nicht überlebt.")
                        time.sleep(1)
                        a.asciimuchskullded()
    ...
    if easteregg=="jupi":
        ...
        a.asciigameover()
        time.sleep(2)
        restart=input("Drücke Enter um ein neues Spiel zu starten...")
